...
var checkToken = expressJwt({ secret: config.secrets.jwt }); // this returns a middleware function
...
exports.decodeToken = function() {
  return function(req, res, next) {
    ...
    // See follow the 'Bearer 034930493' format
...
// use _id to get the whole user
exports.getFreshUser = function() {
  return function(req, res, next) {
    User.findById(req.user._id)
      .then(function(user) {
        if (!user) {
          // if no user is found it was not it was a valid JWT but didn't decode to a real user in our DB.
          // Either the user was deleted since the client got the JWT,
          // or it was a JWT from some other source
      ...
      .then(function(user) {
        if (!user) {
          ...
          if (!user.authenticate(password)) {
            ...
            // and call next so the controllercan sign a token from the req.user._id
            req.user = user;
            next();
          }
        }
